...
import { participantSchedule, scheduleRtf } from '../lib/participantExport'
import { Button } from './ui'

export function ParticipantExport({ project }: { project: Project }) {
  const [id, setId] = useState('')
  const [note, setNote] = useState('')
  const person = [...project.dms, ...project.teams].find((p) => p.id === id)
  const side = project.dms.some((p) => p.id === id) ? 'dm' : 'team'
  const lines = person ? participantSchedule(project, side, id) : []
  ...
  return (
    <section className="mt-6 rounded border border-rule p-3">
      ...
      <p className="mb-3 text-muted">The current board, just for one person or team. Free and unavailable times are included.</p>
